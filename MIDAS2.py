...
class MIDAS(torch.nn.Module):
    ...
    def fit(self, 
        ...
        if col_convert:
            ...
            raise ValueError("If col_convert == False, you must provide col_types and type_dict.")
    ...
    def _build_model(self):
        ...
        for i, hidden_dim in enumerate(self.hidden_layers):
            self.encoder.add_module(f"layer_{i+1}", torch.nn.Linear(prev_dim, hidden_dim))
            self.encoder.add_module(f"dropout_{i+1}", torch.nn.Dropout(self.dropout_prob))
        ...
        for i, hidden_dim in enumerate(decode_layers[:-1]):
            self.decoder.add_module(f"layer_{i+1}", torch.nn.Linear(hidden_dim, decode_layers[i+1]))
            self.decoder.add_module(f"dropout_{i+1}", torch.nn.Dropout(self.dropout_prob))
            self.decoder.add_module(f"relu_{i+1}", torch.nn.ReLU())
        
        self.decoder.add_module("output", torch.nn.Linear(decode_layers[-1], self.input_dim))
        ...
        with torch.no_grad():
            for _ in range(m):
                ...
                for i, col in enumerate(X.col_types):
                    if col == 'bin':
                        imputed.iloc[:,i] =  torch.nn.functional.sigmoid(imputed.iloc[:,i])
                        ...
                        tmp_cat = [X.type_dict[X.col_names[i]][x] for x in np.argmax(imputed.iloc[:,i:i+col].to_numpy(), axis = 1)]
                        imputed.drop([j for j in range(i,i+col)], axis = 1, inplace = True)
